...
public interface IRUService<TEntity, TId> : IRService<TEntity, TId>, IUService<TEntity, TId> where TEntity : class, IEntity<TId>
{
    ...
    Task<TResponse?> UpdateAsync<TResponse, TRequest>(TRequest request) where TResponse : BaseResponse<TEntity, TId>, new() where TRequest : BaseUpdateRequest<TEntity, TId>;
    Task<TResponse[]?> UpdateAsync<TResponse, TRequest>(IEnumerable<TRequest> request) where TResponse : BaseResponse<TEntity, TId>, new() where TRequest : BaseUpdateRequest<TEntity, TId>;
    ...
    Task<TEntity?> UpdateAsync<TRequest>(TRequest request) where TRequest : BaseUpdateRequest<TEntity, TId>;
    Task<TEntity[]?> UpdateAsync<TRequest>(IEnumerable<TRequest> request) where TRequest : BaseUpdateRequest<TEntity, TId>;
}
